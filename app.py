# brayfxtrade_pro_sidebar_analysis.py
...
TF_TO_YF_INTERVAL = {
    "1H": "60m",
    "4H": "60m",  # resample to 4H later
    "1D": "1d",
    "15m": "15m",
    "5m": "5m"
}

# Frontend: normal pair names
FRONTEND_PAIRS = ["EURUSD","GBPUSD","USDJPY","AUDUSD","USDCAD",
                  "USDCHF","NZDUSD","GBPJPY","BTC-USD","ETH-USD"]

# Backend: append '=X' for Yahoo Finance if needed
def backend_pair(frontend):
    if frontend in ["BTC-USD","ETH-USD"]:
        return frontend
    return frontend + "=X"
...
with col1:
    ...
    tv_html = f"""
    <div id="tv_widget" style="height:520px;"></div>
...
if raw_df.empty:
    st.error(f"No data for {pair_front} in {timeframe}. Try a different range or timeframe.")
    st.stop()
...
def simulate_trades(signals, df):
    ...
    for _, s in signals.iterrows():
        ...
        trades.append({"Idx":idx,"Type":typ,"StrategyName":s['StrategyName'],"Entry":entry,"SL":sl,"TP":tp,"Profit":profit,"Win":win})
...
with left:
    ...
    if signals_df.empty:
        st.info("No signals yet. Click ▶️ Run Analysis in sidebar.")
    else:
    ...
    if signals_df.empty:
        ...
        for _, s in latest.iterrows():
            if "Bull" in s['Type']:
                st.success(f"🚀 {s['Type']} ({s['StrategyName']}) @ {s['Price']:.5f}")
            elif "Bear" in s['Type']:
                st.warning(f"⚠️ {s['Type']} ({s['StrategyName']}) @ {s['Price']:.5f}")
            else:
                st.info(f"ℹ️ {s['Type']} ({s['StrategyName']}) @ {s['Price']:.5f}")
...
with right:
    st.subheader("Candlestick Chart + Signals + OB/FVG Zones")
    ...
    if not signals_df.empty:
        for _, s in signals_df.iterrows():
            idx=int(s['Index']); t0=df.index[idx]
            top, bottom= s.get('Top',None), s.get('Bottom',None)
            typ=s['Type']
            ...
            fig.add_trace(go.Scatter(x=[df.index[idx]],y=[s['Price']],mode="markers+text",
                                     ...
                                     text=[typ],textposition="top center",name=f"{typ}"))
    if not trades_df.empty:
