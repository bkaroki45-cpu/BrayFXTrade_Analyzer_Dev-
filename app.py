# brayfxtrade_pro_with_analysis.py
...
with st.sidebar:
    ...
    # Map frontend -> backend symbol for yfinance
    YF_MAP = {
    ...
    pair = YF_MAP[selected_front_pair]
    ...
    use_patterns = st.checkbox("Chart Patterns", value=True)

    st.markdown("---")
    st.subheader("Live / Export Options")
...
if clear_state:
    for k in ['signals_df','trades_df','market_df']:
...
yf_interval = TF_MAP.get(timeframe,"60m")
...
if df.empty:
    st.error(f"No data for {selected_front_pair} in {timeframe}. Try a different range.")
    st.stop()

st.session_state['market_df'] = df

# ---------------------------
# Analysis heuristics (OB/FVG/Patterns)
def detect_ob(data):
    rows=[]
...
def run_analysis(df,use_ob,use_fvg,use_patterns):
    ...
    if use_ob:
        s=detect_ob(df); s['StrategyName']="OB Strategy"; signals=pd.concat([signals,s],ignore_index=True)
    if use_fvg:
        s=detect_fvg(df); s['StrategyName']="FVG Strategy"; signals=pd.concat([signals,s],ignore_index=True)
    if use_patterns:
        s=detect_patterns(df); s['StrategyName']="Pattern Recognition"; signals=pd.concat([signals,s],ignore_index=True)
...
# ---------------------------
# Layout columns: left analysis / right charts
left, right = st.columns([1.2,1.8])

with left:
    st.subheader("Signals")
    if signals_df.empty:
        st.info("No signals detected. Click **Run Analysis**.")
    else:
    ...
    if export_csv:
        if not signals_df.empty:
            st.download_button("⬇️ Download Signals CSV", signals_df.to_csv(index=False).encode('utf-8'),file_name=f"{selected_front_pair}_signals.csv",mime="text/csv")
        if not trades_df.empty:
            st.download_button("⬇️ Download Trades CSV", trades_df.to_csv(index=False).encode('utf-8'),file_name=f"{selected_front_pair}_trades.csv",mime="text/csv")

with right:
    st.subheader("TradingView & Programmatic Chart")
    tv_html=f"""
    <div id="tv_widget" style="height:600px;"></div>
