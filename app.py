...
TF_TO_YF_INTERVAL = {
    "1H": "60m",
    "4H": "60m",   # will resample to 4H from 60m
    "1D": "1d",
    "15m": "15m",
    "5m": "5m"
}

# Frontend pair names (clean)
CLEAN_PAIRS = [
    "EURUSD","GBPUSD","USDJPY","AUDUSD","USDCAD",
    "USDCHF","NZDUSD","GBPJPY","BTC-USD","ETH-USD"
]

# Backend mapping for Yahoo Finance
BACKEND_PAIRS = {
    "EURUSD":"EURUSD=X","GBPUSD":"GBPUSD=X","USDJPY":"USDJPY=X",
    "AUDUSD":"AUDUSD=X","USDCAD":"USDCAD=X","USDCHF":"USDCHF=X",
    "NZDUSD":"NZDUSD=X","GBPJPY":"GBPJPY=X","BTC-USD":"BTC-USD","ETH-USD":"ETH-USD"
}
...
with st.sidebar:
    ...
    clean_pair = st.selectbox("Select Forex Pair", CLEAN_PAIRS, index=2)
    pair = BACKEND_PAIRS[clean_pair]  # backend uses Yahoo symbol
...
# ---------------------------
# TradingView widget + Quick Stats
...
if df.empty:
    st.error(f"No data for {clean_pair} — try a different timeframe or shorter date range.")
    st.stop()
...
def detect_ob(data):
    ...
    for i in range(1,len(data)-1):
        ...
        if cur.Close<cur.Open and body_cur>body_prev and body_cur>0.8*body_next:
...
with col1: run_now = st.button("▶️ Run Analysis")
with col2: clear_state = st.button("🧹 Clear Results")

if clear_state:
    for k in ['signals_df','trades_df']: st.session_state.pop(k,None)
...
with left:
    ...
    if signals_df.empty: st.info("No signals detected yet. Click **Run Analysis** to scan the chosen range.")
    else:
        display = signals_df.copy()
        display['Time'] = display['Index'].apply(lambda i: df.index[int(i)].strftime("%Y-%m-%d %H:%M"))
        st.dataframe(display[['Time','Type','StrategyName','Price']].sort_values(by='Time',ascending=False),height=300)
    st.markdown("### Performance Summary")
    if trades_df.empty: st.info("No trades simulated yet.")
    else:
        summary = trades_df.groupby('StrategyName').agg(Total_Trades=('Win','count'),Total_Profit=('Profit','sum'),Win_Rate=('Win','mean'),Avg_Profit=('Profit','mean')).reset_index()
    ...
    if export_csv:
        if not signals_df.empty:
            st.download_button("⬇️ Download Signals CSV",signals_df.to_csv(index=False).encode('utf-8'),file_name=f"{clean_pair}_signals.csv",mime="text/csv")
        if not trades_df.empty:
            st.download_button("⬇️ Download Trades CSV",trades_df.to_csv(index=False).encode('utf-8'),file_name=f"{clean_pair}_trades.csv",mime="text/csv")

with right:
    st.subheader("Programmatic Chart (candles + signals + OB/FVG zones)")
    fig = go.Figure(data=[go.Candlestick(x=df.index,open=df['Open'],high=df['High'],low=df['Low'],close=df['Close'],increasing_line_color='green',decreasing_line_color='red',name='Price')])
    if not signals_df.empty:
        for _,s in signals_df.iterrows():
            idx=int(s['Index'])
            if idx<0 or idx>=len(df): continue
            t0=df.index[idx]; top=s.get('Top'); bottom=s.get('Bottom'); typ=s['Type']
            color='rgba(0,255,0,0.15)' if 'Bull' in typ else 'rgba(255,0,0,0.12)'
            if top is not None and bottom is not None:
                fig.add_shape(type="rect",x0=t0-pd.Timedelta(minutes=1),x1=t0+pd.Timedelta(hours=12),y0=bottom,y1=top,fillcolor=color,line=dict(width=0),layer="below")
            fig.add_trace(go.Scatter(x=[df.index[idx]],y=[s['Price']],mode="markers+text",marker=dict(color='green' if 'Bull' in typ else 'red',size=10),text=[s['Type']],textposition="top center",name=f"{s['Type']}"))
    if not trades_df.empty:
        fig.add_trace(go.Scatter(x=[df.index[min(int(r['Idx']),len(df)-1)] for _,r in trades_df.iterrows()],y=trades_df['CumulativeProfit'],mode="lines+markers",name="Equity",yaxis="y2"))
        fig.update_layout(yaxis2=dict(overlaying="y",side="right",title="Cumulative Profit",showgrid=False))
    fig.update_layout(margin=dict(l=10,r=10,t=30,b=10),height=700,legend=dict(orientation="h",yanchor="bottom",y=1.02,xanchor="right",x=1))
